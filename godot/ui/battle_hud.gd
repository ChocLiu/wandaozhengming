...
func update_state(battle) -> void:
	...
	if actor == battle.player:
		hint = "\n▶ 轮到你：点棋盘空格移动（剩%d步）→ 普攻随机出招 / 「招式」手动选招 / 「守势」保护部位+架势 / 「切换」换功法 / 丹药" % actor.move_left
	info_label.text = "【你】%s\n【对手】%s\n当前行动：%s%s" % [
...
func _unit_line(u: Unit) -> String:
	...
	var s := "%s（%s·主修《%s》·出招《%s》·%s）气血%d/%d 体力%d/%d 玄力%d/%d 魂力%d/%d 攻击%.0f 防御%.0f 招架%.0f 止血丹×%d" % [
		u.display_name, u.realm, main_name, tech_name, wname,
	...
	for part in u.body:
		...
		parts.append("%s[%s%s]%s" % [part, BodySystem.STATE_NAMES[pd.state], "·流血" if pd.bleeding else "", "·主" if part == u.main_arm else ""])
	s += "\n部位: " + " ".join(parts)
	# 守势（§5.1.2）
	if u.stance != "":
		s += "\n守势: %s·[%s]" % [u.stance, " ".join(u.guard_parts)]
	# 状态
	var st: Array[String] = []
