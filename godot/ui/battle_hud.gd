...
## 面板图排版统一规范（2026-09-02 定）：四边各 5% 为卷轴轴头/花边装饰区，内容安全区 = 中央 90%×90%
const PANEL_EDGE_RATIO := 0.05
...
func _ready() -> void:
	...
	# 内容缩进：四边各 5%（横 1256×0.05≈63，竖 56×0.05≈3）——卷轴轴头区不放内容
	info_label.position = Vector2(63.0, 2.8)
	info_label.size = Vector2(1130, 50.4)
	...
	# 内容缩进：四边各 5%（横 632×0.05≈32，竖 420×0.05=21）——卷轴轴头区不放内容
	log_rtl.position = Vector2(32.0, 21)
	log_rtl.size = Vector2(568, 378)
...
func _make_button(text: String, on_pressed: Callable) -> Button:
	...
	# 内容缩进：四边各 5%（横 96×0.05≈5，竖 36×0.05≈2）——按钮文字/图标自动让出卷轴轴头区
