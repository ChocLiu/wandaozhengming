...
**状态：P0 战斗原型 v0.0.6（2026-09-05 场地机制版——太虚布阵 + 焚天火势循环）。** 渲染器：Compatibility（gl_compatibility）——2D 回合制项目推荐，若将来需要 2D 辉光等特效可一键切换 Forward+。
...
- **场地机制（v0.0.6）**：焚天火势循环（火浪/爆炎命中点火 fire=2、燃血耗气血强点火 fire=3 自燃、火区确定性蔓延 fire≥2 向四邻扩散衰减、行动者脚下 fire×2 直伤+灼烧不经防御链、敌我双刃剑）+ 太虚阵经（太虚点布节点双方可见、踏入触发 6 伤+麻痹、三连成阵三节点爆发）——《战斗系统》§7.1；玩家招式槽挂太虚阵经（战斗内可切换）

待实现（按路线图 P0 剩余项）：
- 散招与通用大招（青莲大招已随剑意落地；招式槽散招后置）
- 神魂击杀途径、更多状态（麻痹已随踏阵落地）
